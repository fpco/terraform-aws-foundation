...
resource "aws_ebs_volume" "volume0" {
  count             = var.sigpos[signum(length(split(",", var.sizes)))]
...
resource "aws_volume_attachment" "volume0" {
  count       = var.sigpos[signum(length(split(",", var.sizes)))]
  device_name = "/dev/sdf"
  volume_id   = aws_ebs_volume.volume0[0].id
  instance_id = var.instance_id
}

resource "aws_ebs_volume" "volume1" {
  count             = var.sigpos[signum(length(split(",", var.sizes)) - 1)]
  availability_zone = var.availability_zone
...
resource "aws_ebs_volume" "volume2" {
  count             = var.sigpos[signum(length(split(",", var.sizes)) - 2)]
  availability_zone = var.availability_zone
...
resource "aws_volume_attachment" "volume2" {
  count       = var.sigpos[signum(length(split(",", var.sizes)) - 2)]
  device_name = "/dev/sdh"
  volume_id   = aws_ebs_volume.volume2[0].id
  instance_id = var.instance_id
}

resource "aws_ebs_volume" "volume3" {
  count             = var.sigpos[signum(length(split(",", var.sizes)) - 3)]
  availability_zone = var.availability_zone
...
resource "aws_volume_attachment" "volume3" {
  count       = var.sigpos[signum(length(split(",", var.sizes)) - 3)]
  device_name = "/dev/sdi"
  volume_id   = aws_ebs_volume.volume3[0].id
  instance_id = var.instance_id
}

resource "aws_ebs_volume" "volume4" {
  count             = var.sigpos[signum(length(split(",", var.sizes)) - 4)]
  availability_zone = var.availability_zone
...
resource "aws_volume_attachment" "volume4" {
  count       = var.sigpos[signum(length(split(",", var.sizes)) - 4)]
  device_name = "/dev/sdj"
  volume_id   = aws_ebs_volume.volume4[0].id
  instance_id = var.instance_id
}
