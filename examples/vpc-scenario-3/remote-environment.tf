...
resource "aws_vpc_peering_connection" "vpc-to-vpc2" {
  peer_vpc_id = "${module.vpc2.vpc_id}"
  vpc_id      = "${module.vpc.vpc_id}"
  auto_accept = true

  tags {
    Name = "${var.name}-vpc-to-vpc2"
  }
}

resource "aws_route" "vpc-to-vpc2" {
  route_table_id            = "${module.vpc-public-gateway.route_table_id}"
  destination_cidr_block    = "${module.vpc2.vpc_cidr_block}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.vpc1-to-vpc2.id}"
}

resource "aws_route" "vpc2-to-vpc" {
